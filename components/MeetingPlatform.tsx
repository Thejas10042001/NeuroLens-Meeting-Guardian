    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[600px] gap-8">
        ...
        <div className="mt-8 text-center text-gray-500 text-sm max-w-md">
            <p>ℹ️ <strong>Note for Testing:</strong> This uses "Local" WebRTC. To test peer connections, open this app in <strong>two separate browser tabs</strong>.</p>
        </div>
      </div>
    );
  }
                    ...
                    {participants.map(p => (
                        <div key={p.id} className={`flex items-center justify-between p-2 rounded border ${p.status === 'kicked' ? 'bg-red-900/10 border-red-900' : 'bg-gray-800/50 border-gray-700/50'}`}>
                            ...
                            <div className="flex gap-1">
                                {!p.hasAudio && p.status !== 'kicked' && <MicOffIcon />}
                                {p.metrics.stress > 80 && p.status !== 'kicked' && <span title="High Stress" className="text-red-400 text-xs">⚠</span>}
                            </div>
                        </div>
                    ))}
                </div>
